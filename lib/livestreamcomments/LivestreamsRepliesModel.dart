...
class LivestreamsRepliesModel with ChangeNotifier {
  List<Replies> _items = [];
  bool isError = false;
  int commentId = 0;
  ...
  LivestreamsRepliesModel(BuildContext context, int commentId,
      Userdata userdata, int totalCommentsReply) {
    _context = context;
  ...
  Future<void> fetchComments() async {
    ...
    try {
      final response = await http.post(Uri.parse(ApiUrl.loadlivetvreplies),
          body: jsonEncode({
  ...
  Future<void> constructComment(String content) async {
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.replylivetvcomment),
  ...
  Future<void> deleteComment(int commentId, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.deletelivetvreply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> editComment(int id, String content, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.editlivetvreply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
