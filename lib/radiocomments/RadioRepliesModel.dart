...
class RadioRepliesModel with ChangeNotifier {
  List<Replies> _items = [];
  bool isError = false;
  int commentId = 0;
  ...
  RadioRepliesModel(BuildContext context, int commentId, Userdata userdata,
  ...
  Future<void> fetchComments() async {
    ...
    try {
      final response = await http.post(Uri.parse(ApiUrl.loadradioreplies),
          body: jsonEncode({
  ...
  Future<void> constructComment(String content) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.replyradiocomment),
  ...
  Future<void> deleteComment(int commentId, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.deleteradioreply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> editComment(int id, String content, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.editradioreply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> reportComment(int commentId, int position, String reason) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.reportradiocomment),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
