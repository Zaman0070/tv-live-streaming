...
class MediaRepliesModel with ChangeNotifier {
  List<Replies> _items = [];
  bool isError = false;
  int commentId = 0;
  ...
  MediaRepliesModel(BuildContext context, int commentId, Userdata userdata,
  ...
  Future<void> fetchComments() async {
    try {
      final response = await http.post(Uri.parse(ApiUrl.loadmediareplies),
          body: jsonEncode({
  ...
  Future<void> constructComment(String content) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.replymediacomment),
  ...
  Future<void> deleteComment(int commentId, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.deletemediareply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> editComment(int id, String content, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.editmediareply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> reportComment(int commentId, int position, String reason) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(ApiUrl.reportmediacomment),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
